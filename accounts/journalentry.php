...
<div class="entryscreen" style="display:none;">
	...
	<form  action=""   method="post" id="journalform"  class="form-horizontal">
    ...
    <table class="table">
        ...
        <tr>
			<td>
			    <button type="button" hidden id="another"   name="another"   class="btnDeleteAction" >Add</button>
...
<div class="mainscreen"  >
    <div class="col-sm-2">	
	   <button type="button" id="entry"  class="btnDeleteAction" >Add New-Journal</button> 
	</div>	
   <div class="form-group">
    <form id="rock" action="<?php echo base_url();?>accounts/journalentry" method="post" enctype="multipart/form-data">  
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
		    ...
		    <?php
			    ...
			    foreach($castxe->result() as $data)
				  {
				   echo "<input disabled type='radio' name='comp_id'  id='$data->comp_id' class='comp_id' value='$data->comp_id'";
				   echo ">$data->comp_name</input>";					
				 } 
		    ?>		 
           </div> 
        
		<input type="text" readonly   value ="Accounts From" class="form-control"> 
		<div class="form-group has-feedback">
...
//////////////////////////
  ...
  $('#exampleModal').on('show.bs.modal', function (e) {
	...
	$.ajax({
		url: '<?php echo site_url("accounts/getaccbillingjournal")?>',
		type: "POST",
		data:'vch_no='+vch_no, 
...
{ 
	...
	var accodeto=$('#accodeto').val();
	 ...
	 if (accodefrom===accodeto)
	 {
		 alert('Both Accounts Same!!!!!!!!');
		 return false;
	 }
	var comp_idd = $("input[name='comp_idd']:checked").val();
	 
	$.ajax({
		        url: '<?php echo site_url("accounts/savejournal")?>',
		        type:'POST',
				async: false,
...
function getsaveaccounts(action) {
    ...
    var editaccodeto =$('#editaccodeto :selected').val(); 
	 ...
	 if (editaccodefrom===editaccodeto)
	 {
		 alert('Both Accounts Same!!!!!!!!');
		 return false;
	 }
	...
	url: '<?php echo site_url("accounts/editsavebilljournal")?>',
