...
class Client extends CI_Controller {
   ...
   public function index() {
       ...
       //$services= $this->input->post('subjob'); print_r($services);	exit;
		...
		else if (strcasecmp( $flag, 'EDIT' ) == 0 )
		{
		  ...
		  $strSQL ="update accmasaccounts set ac_name='".$ac_name."',address='".$address."',
			        pan='".$pan."',tin_no='".$tin_no."',contactperson='".$contactperson."',
...

	{
        $id=$_POST["id"];
		$column=$_POST["column"];		
        $strSQL ="update accmasaccounts set $column='".$_POST["editval"]."'    where ac_code='".$id."'";						  
	    $this->db->query($strSQL);  
