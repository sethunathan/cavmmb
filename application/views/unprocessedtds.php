...
#btnAddAction{background-color:#09F;border:0;padding:5px 10px;color:#FFF;}
 ...
 <li class="active treeview">
          <a href="<?php echo base_url(); ?>admin/unprocessedtds">
            <i class="fa fa-dashboard"></i> <span>Un Processed  T.D.S - Refresh</span>
...
<form action="<?php echo base_url(); ?>admin/unprocessedtds" method="post">
...
<?php $i=1; foreach ($client as $rsclient) :  ?>  
	...
	onBlur="saveitreturndate(this,'itreturndate','<?php  echo $rsclient['id'] ?>','<?php  echo $rsclient['ac_name'] ?>')"
	onClick="showEdit(this);"><?php echo date("d-m-Y", strtotime($rsclient['itreturndate'] )); ?></td>
	...
	onClick="showEdit(this);"><?php echo $rsclient['remarks'] ; ?></td>
	   ...
	   <td> 
	 <button class="btnEditAction" name="tdsdefective"
      onClick="callCrudAction('tdsdefective','<?php echo $rsclient['id']; ?>','<?php echo $rsclient['ac_name']; ?>')">
...
function callCrudAction(action,id,acname) {
	...
	success:function(data){
		switch(action) {
			case "tdsdefective":
				 $('#message_'+id).fadeOut();
				 $("#txtmessage").val(acname+' Sucessfully Moved to Defective T.D.S.List ');
			break;
            case "tdsform16":
				 $('#message_'+id).fadeOut();
