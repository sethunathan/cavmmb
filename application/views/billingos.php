...
$total=0;
 foreach ($client as $rsclient) :  ?>  
	...
	<td ><?php $opnbal=$rsclient['opn_bal'];
	    if ($opnbal<0){ echo abs($rsclient['opn_bal']).' Dr';}  
		else if ($opnbal>0){ echo abs($rsclient['opn_bal']).' Cr';}  
	    ?></td> 
	...
	<?php
	     ...
	     if ($closbal<0){ echo abs($rsclient['clos_bal']).' Dr';} 	
	     else if ($closbal>0){ echo abs($rsclient['clos_bal']).' Cr';} 	
		 $drtotal= $drtotal+($rsclient['tot_dr']);
		 $crtotal= $crtotal+($rsclient['tot_cr']); 
		 $total= $total+($rsclient['clos_bal']);
	?></td>	
...
if ($total<0){ echo abs($total).' Dr';}  
		else if ($total>0){ echo $total.' Cr';} 	
