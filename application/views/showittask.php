...
function callCrudAction(action,id,acname) {
	...
	success:function(data){
		switch(action) {
			case "itverified":
				 $('#message_'+id).fadeOut();
				 $("#txtmessage").val(acname+' Sucessfully Moved to Verified I.T.List ');
			break;			
...
#btnAddAction{background-color:#09F;border:0;padding:5px 10px;color:#FFF;}
 ...
 <li class="active treeview">
          <a href="<?php echo base_url(); ?>admin/linkit">
            <i class="fa fa-dashboard"></i> <span>Un Verfied  I.T.R - Refresh</span>
...
<?php $i=1; foreach ($client as $rsclient) :  ?>  
	...
	<?php echo substr($rsclient['ac_name'],0,20).'-'.$rsclient['id'] ?></td>
	<td><?php echo $rsclient['pan'] ; ?></td>	
	<td><?php echo $rsclient['task_name'].'-'.$rsclient['subjobname'] ; ?></td>	
	
	<td contenteditable="true"
	onBlur="saveDate(this,'itreturndate','<?php  echo $rsclient['id'] ?>','<?php  echo $rsclient['ac_name'] ?>')"
	onClick="showEdit(this);">
	<?php 
	
	echo date("d-m-Y", strtotime($rsclient['itreturndate'] )); ?>
	</td>
    ...
    <td> 
	 <button class="btnEditAction" name="itverified"
      onClick="callCrudAction('itverified','<?php echo $rsclient['id']; ?>','<?php echo $rsclient['ac_name']; ?>')">
      Move to Verified List</button> 
      </td>
	  <td>
<?php 
...
function saveToDatabase(editableObj,column,id,acname) {
  ...
  <script>
function saveDate(editableObj,column,id,acname) {
	...
	$.ajax({
		...
		success: function(data){
			 ...
			 $("#loaderIcon").hide();
			 $("#txtmessage").val(acname+' Date Sucessfully Updated !!!'+data);
