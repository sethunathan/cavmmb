...
#btnAddAction{background-color:#09F;border:0;padding:5px 10px;color:#FFF;}
 ...
 <li class="active treeview">
          <a href="<?php echo base_url(); ?>auditingfollowup">
...
<form action="<?php echo base_url(); ?>auditingfollowup" method="post">
...
function saveToDatabase(editableObj,column,id,acname) {
	...
	$.ajax({
		...
		success: function(data){
			 $("#loaderIcon").hide();
			 $("#txtmessage").val(acname+' Auditing-Entry Level Remarks Sucessfully Updated !!!');
		}        
   });
}
</script>
...
function callCrudAction(action,id,acname) {
	...
	jQuery.ajax({
	url: '<?php echo site_url("auditingfollowup/movetoentrylevel")?>',
