...
<form action="<?php echo base_url(); ?>itfollowup" method="post">
...
function saveToDatabase(editableObj,column,id,acname) {
	...
	$.ajax({
		...
		success: function(data){
			 $("#loaderIcon").hide();
			 $("#txtmessage").val(acname+' IT-Entry Level Remarks Sucessfully Updated !!!');
		}        
   });
}
</script>
...
function callCrudAction(action,id,acname) {
	...
	jQuery.ajax({
	url: '<?php echo site_url("itfollowup/movetoentrylevel")?>',
