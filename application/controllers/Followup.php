...
class Followup extends CI_Controller {
	...
	{	
		...
		if (isset($task_code))
			{
			  ...
			  $this->db->where('trntask2.task_code', $task_code);
			  $data['followupp'] =$task_code;
        }
	   ...
	   // $this->db->where('trntask2.movetotask =',0); 
	    ...
	    $this->db->select('empname,followupdoneat,movetotask,nextreminderat,task_name,ac_name,trntask2.ac_code,
        ...
        $this->load->view('followupalterstatus',$data);	
	}
	...
	{	
	   ...
	   $data['followupp'] = 0;
	    ...
	    $this->db->select('childtask_code,gstid,gstpwd,empname,followupdoneat,nextreminderat,task_name,ac_name,trntask2.ac_code,
		trntask2.task_code,accmasaccounts.contactno,trntask2.remarks,trntask2.id,gstid,gstpwd,remindername,reminder_date,
		  accmasaccounts.group_code,clarificationremarks,clarificationreverse  ');
	...
	{	
	   ...
	   $loadbill =0;
		...
		if ($loadbill==1)
		{
					$this->db->where('billed', $loadbill);//sethu 08 05 2019
		}

	    $this->db->order_by('trntask3.reminder_code,reminder_date,followupdoneat,nextreminderat,
		trntask2.id,trntask1.task_code,ac_name,trntask2.ac_code');
		
	    $this->db->select('childtask_code,trntask1.subjob_code,empname,followupdoneat,nextreminderat,task_name,ac_name,trntask2.ac_code,
		trntask2.task_code,accmasaccounts.contactno,trntask2.remarks,trntask2.id,gstid,gstpwd,remindername,reminder_date,
		  accmasaccounts.group_code,clarificationremarks,clarificationreverse  ');
...

	{
       ...
       echo $verificationassignto=$_POST["emp_code"];			
	...
	private function stripHTMLtags($str)
{
	...
	{	
	    ...
	    $this->db->order_by('entrydoneat,nameid,trntask2.id'); //order by
		$this->db->where('job_code', 4);	
		 
	     
		$this->db->select('entrydoneat,childtask_code, DATEDIFF(STR_TO_DATE(CONCAT(SUBSTRING(entrydoneat,1,2), "-",SUBSTRING(entrydoneat,4,2), "-",SUBSTRING(entrydoneat,7,4)),"%d-%m-%Y"),NOW()) AS nameid');
